...
body {
    ...
    margin: 0;
    padding: 20px;
    background: linear-gradient(rgba(0,0,0,0.9), rgba(0,0,0,0.9)), url('https://wallpapercave.com/wp/wp2797149.jpg') no-repeat center center fixed;
...
.character-img { flex: 1 1 300px; }
.character-img img {
    max-width: 100%;
    border-radius: 8px;
    box-shadow: 0 0 20px rgba(0,0,0,0.6);
}
.character-info { flex: 2 1 500px; }
.info-grid {
    display: grid;
...
.description { text-align: justify; line-height: 1.6; }
.container.cortana .description { color: #c8e9ff; }
.container.jefe .description { color: #d4ffd4; }
</style>
</head>
<body>
<div class="top-bar"><a href="index.php">Volver al inicio</a></div>
...
while ($row = $resultado->fetch_assoc()) {
    ...
    echo "<div class='character-img'><img src='" . htmlspecialchars($row["imagen"]) . "'></div>";
    ...
    echo "<div><strong>Altura:</strong> " . htmlspecialchars($row["altura"]) . " m</div>";
    ...
    echo "</div><p class='description'>" . nl2br(htmlspecialchars($row["descripcion"])) . "</p></div></div>";
}

// Mostrar AIs
$sqlCortana = "SELECT * FROM InteligenciasArtificiales WHERE categoria = 'Jefe Maestro y Cortana'";
$resultadoCortana = $conexion->query($sqlCortana);
